  ...
  local divide = function(x)
    return function(y)
      if y == 0 then
        ...
        _raise(setmetatable({tag = DivideByZero_tag, payload = "divide " .. tmp1 .. " by zero"}, _exn_meta), "exc_4.sml:3:18")
      else
        return _Int_div(x, y)
      end
    end
  end
  local tmp1 = divide(10)
  local x = tmp1(5)
  local tmp2 = gsub(tostring(x), "-", "~")
  outputAndFlush(stdOut, tmp2)
  outputAndFlush(stdOut, "\n")
  tmp = divide(10)
end
local y = tmp(0)
local tmp1 = gsub(tostring(y), "-", "~")
