...
local outputAndFlush, stdOut, Yellow, White, Dark, Bright, rgb_as_string, to_rgb
do
  local io = _ENV.io
  ...
  to_rgb = function(a)
    if a.tag == "BasicColor" then
      ...
      elseif color == "White" then
        ...
        _raise(_Match, "colors_5.sml:39:5")
      end
      ...
      elseif brightness_level == "Bright" then
        ...
        _raise(_Match, "colors_5.sml:34:5")
      end
    ...
    elseif a.tag == "RGB" then
      ...
      _raise(_Match, "colors_5.sml:57:5")
    end
  end
  local tmp
  do
...
local tmp
do
  local tmp1 = to_rgb({tag = "BasicColor", payload = {Yellow, Dark}})
  local r = tmp1[1]
  local g = tmp1[2]
  tmp = rgb_as_string(r, g, tmp1[3])
end
...
local tmp1
do
  local tmp2 = to_rgb({tag = "BasicColor", payload = {Yellow, Bright}})
  local r = tmp2[1]
  local g = tmp2[2]
  tmp1 = rgb_as_string(r, g, tmp2[3])
end
...
local tmp2
do
  local tmp3 = to_rgb({tag = "BasicColor", payload = {White, Dark}})
  local r = tmp3[1]
  local g = tmp3[2]
...
local tmp3
do
  local tmp4 = to_rgb({tag = "BasicColor", payload = {White, Bright}})
  local r = tmp4[1]
  local g = tmp4[2]
...
local tmp4
do
  local tmp5 = to_rgb({tag = "Gray", payload = 128})
  local r = tmp5[1]
  local g = tmp5[2]
...
local tmp5
do
  local tmp6 = to_rgb({tag = "Gray", payload = 255})
  local r = tmp6[1]
  local g = tmp6[2]
...
local tmp6
do
  local tmp7 = to_rgb({tag = "RGB", payload = {10, 20, 30}})
  local r = tmp7[1]
  local g = tmp7[2]
